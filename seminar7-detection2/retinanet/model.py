...
class ResNet(nn.Module):
    ...
    def freeze_bn(self):
        """allowing the batchnorm layers to update is only valuable if the batch size of the data being passed
        through is large. Otherwise it adjusts to specific images each iteration, which gives inaccurate updates."""
    ...
    def forward(self, inputs):
        ...
        regression = torch.cat([self.regressionModel(feature) for feature in features], dim=1) # one model for each layer

        classification = torch.cat([self.classificationModel(feature) for feature in features], dim=1) # one model for each layer
